...
void RunPipelineSteps(const std::shared_ptr<KamaletdinovRBitwiseIntOMP> &task) {
  EXPECT_TRUE(task->Validation());
  EXPECT_TRUE(task->PreProcessing());
  EXPECT_TRUE(task->Run());
}

void RunTaskPipeline(const std::shared_ptr<KamaletdinovRBitwiseIntOMP> &task, int expected) {
...
const auto kTestTasksList = std::tuple_cat(ppc::util::AddFuncTask<KamaletdinovRBitwiseIntOMP, InType>(
    kTestParam, PPC_SETTINGS_kamaletdinov_r_bitwise_int_omp));
...
TEST(KamaletdinovRBitwiseIntEdge, PipelineSingleElement) {
  RunTaskPipeline(std::make_shared<KamaletdinovRBitwiseIntOMP>(1), 1);
}

TEST(KamaletdinovRBitwiseIntEdge, PipelineTwoElements) {
  RunTaskPipeline(std::make_shared<KamaletdinovRBitwiseIntOMP>(2), 2);
}

TEST(KamaletdinovRBitwiseIntEdge, PipelineLargerInput) {
  RunTaskPipeline(std::make_shared<KamaletdinovRBitwiseIntOMP>(100), 100);
}
