...
class KarpichIBitwiseBatcherFuncTestsThreads : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    {
      std::string abs_path = ppc::util::GetAbsoluteTaskPath(std::string(PPC_ID_karpich_i_bitwise_batcher_seq), "pic.ppm");
...
const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<KarpichIBitwiseBatcherSEQ, InType>(kTestParam, PPC_SETTINGS_karpich_i_bitwise_batcher_seq));
                   

const auto kGtestValues = ppc::util::ExpandToValues(kTestTasksList);

const auto kPerfTestName = KarpichIBitwiseBatcherFuncTestsThreads::PrintFuncTestName<KarpichIBitwiseBatcherFuncTestsThreads>;
